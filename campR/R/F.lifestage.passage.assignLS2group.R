...
#' 
#' @description 2 life stage groups are fit and the program will decide if weight will be used or not
#' 
...
#' 
#' @details 2 life stage groups are fit and the program will decide if weight will be used or not
#' 
#' @return jared
#' 
#' @author WEST Inc.
...
#' reclassifyFL <- FALSE
#' 
#' F.lifestage.passage.assignLS2group(site,taxon,min.date,max.date,output.file,ci,autoLS,reclassifyFL)
#' 
#' }
F.lifestage.passage.assignLS2group <- function(site, taxon, min.date, max.date, output.file, ci=TRUE,autoLS=TRUE,reclassifyFL=FALSE){

  ## 2 life stage groups are fit and the program will decide if weight will be used or not
  passageWithLifeStageAssign(site=site,taxon=taxon,min.date=min.date,max.date=max.date,output.file=output.file,ci=ci,nLS=2,weightUse=NULL,autoLS=TRUE,reclassifyFL=FALSE)
