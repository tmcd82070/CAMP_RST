...
#' 
#' @description 3 life stage groups are fit and the program will decide if weight will be used or not
#' 
...
#' 
#' @details 3 life stage groups are fit and the program will decide if weight will be used or not
#' 
#' @return jared
#' 
#' @author WEST Inc.
...
#' reclassifyFL <- FALSE
#' 
#' F.lifestage.passage.assignLS3group(site,taxon,min.date,max.date,output.file,ci,autoLS,reclassifyFL)
#' 
#' }
F.lifestage.passage.assignLS3group <- function(site, taxon, min.date, max.date, output.file, ci=TRUE,autoLS=TRUE,reclassifyFL=FALSE){

  ## 3 life stage groups are fit and the program will decide if weight will be used or not
  passageWithLifeStageAssign(site=site,taxon=taxon,min.date=min.date,max.date=max.date,output.file=output.file,ci=ci,nLS=3,weightUse=NULL,autoLS=TRUE,reclassifyFL=FALSE)
