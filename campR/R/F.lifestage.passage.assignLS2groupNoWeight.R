...
#' reclassifyFL <- FALSE
#' 
#' F.lifestage.passage.assignLS2NoWeight(site,taxon,min.date,max.date,
#'   output.file,ci,autoLS,reclassifyFL)
#' 
#' }
F.lifestage.passage.assignLS2groupNoWeight <- function(site, taxon, min.date, max.date, output.file, ci=TRUE,autoLS=TRUE,reclassifyFL=FALSE){
  
  ## 2 life stage groups are fit and weight will not be used for assigning lifestage
  passageWithLifeStageAssign(site=site,taxon=taxon,min.date=min.date,max.date=max.date,output.file=output.file,ci=ci,nLS=2,weightUse=FALSE,autoLS=TRUE,reclassifyFL=FALSE)
