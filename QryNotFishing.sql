...
--
-- The table identifies gaps in sampling down to the minute and includes the trapPossitionID and Date so that it can be used in conjuction with the tempSamplingSummary table.
--
...
-- 	* A copy of the table is attached (or should be)
...
WHERE (((TempSamplingSummary.visitTypeID)>1) AND ((TempSamplingSummary.includeCatchID)=1));
...
UPDATE TempNonSamplingSummary INNER JOIN NonTrapSample1 ON (TempNonSamplingSummary.trapPositionID = NonTrapSample1.trapPositionID) AND (TempNonSamplingSummary.OrderNonSample = NonTrapSample1.OrderNonSample) SET TempNonSamplingSummary.timepreviousSampleEnd = CDate(NonTrapSample1!timePreviousSampleEnd);
...
UPDATE TempNonSamplingSummary SET TempNonSamplingSummary.TotalNonSampleHours = DateDiff('n',TempNonSamplingSummary!timepreviousSampleEnd,TempNonSamplingSummary!timeSampleStarted)\'60' & Format(DateDiff('n',TempNonSamplingSummary!timepreviousSampleEnd,TempNonSamplingSummary!timeSampleStarted) Mod 60,'\:00'), TempNonSamplingSummary.TotalNonSampleMinutes = DateDiff('n',TempNonSamplingSummary!timepreviousSampleEnd,TempNonSamplingSummary!timeSampleStarted)
