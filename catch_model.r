...
#   i.e., "impute" a value for catch when it is missing
#
#   input:
...
#   period.  sampleStart and sampleEnd for each of the new lines are defined so that no gap appears.  Variable
#   'gamEstimated' is true for these new lines. Batch date is assigned based on sampleEnd, as usual. 
...
sunset  <- as.POSIXct( paste(format(catch.df$sampleStart, "%Y-%m-%d"), "19:00:00"), format="%Y-%m-%d %H:%M:%S" )
sunrise <- as.POSIXct( paste(format(catch.df$sampleEnd, "%Y-%m-%d"), "07:00:00"), format="%Y-%m-%d %H:%M:%S" )

catch.df$night <- 0
catch.df$pct.night <- 0


ind <- (catch.df$sampleStart <= sunset) & (sunrise <= catch.df$sampleEnd)
catch.df$night[ind] <- 1
catch.df$pct.night[ind] <- 1

ind <- (catch.df$sampleStart <= sunset) & (sunrise > catch.df$sampleEnd)
catch.df$night[ind] <- 1
catch.df$pct.night[ind] <- (as.numeric(catch.df$sampleEnd[ind]) - as.numeric(sunset[ind])) / (as.numeric(sunrise[ind]) - as.numeric(sunset[ind]))

ind <- (catch.df$sampleStart > sunset) & (sunrise <= catch.df$sampleEnd)
catch.df$night[ind] <- 1
catch.df$pct.night[ind] <- (as.numeric(sunrise[ind]) - as.numeric(catch.df$sampleStart[ind])) / (as.numeric(sunrise[ind]) - as.numeric(sunset[ind]))
...
catch.df <- catch.df[ catch.df$sampleLengthHrs > 0, ]

catch.df$log.sampleLengthHrs <- log(as.numeric( catch.df$sampleLengthHrs ))
...
if( sum(!is.na(catch.df$n.tot) & (catch.df$n.tot > 0)) > 10 ){
    ...
    repeat{
        ...
        if( cur.df == 1 ){
            j <- as.numeric( format(catch.df$sampleEnd, "%j"))
            bs.sEnd <- matrix(j,ncol=1)
        } else if( cur.df == 2 ){
            j <- as.numeric( format(catch.df$sampleEnd, "%j"))
            bs.sEnd <- cbind(j, j*j)
        } else {
            bs.sEnd <- bs( catch.df$sampleEnd, df=cur.df )
        }
...
repeat{
    ...
    if( catch.df$sampleGapLenHrs[i] <= max.ok.gap ){
        i <- i + 1
    } else {
        ...
        sEnd <- catch.df$sampleEnd[i] + cumsum(i.gapLens * 60*60)
        class(sEnd) <- class(catch.df$sampleEnd)
        attr(sEnd, "tzone") <- attr(catch.df$sampleEnd[i], "tzone")   

        sStart <- catch.df$sampleEnd[i] + cumsum( c(0,i.gapLens[-ng]) * 60*60 )
...
#        print(length(i.gapLens))
        ...
        new$n.tot <- pred
        new$sampleLengthHrs <- i.gapLens
        new$sampleGapLenHrs <- 0
        new$sampleEnd <- sEnd
        new$sampleStart <- sStart
        new$gamEstimated <- TRUE
        new$siteID <- catch.df$siteID[i]
        new$trapPositionID <- catch.df$trapPositionID[i]
        new <- F.assign.batch.date( new )     
        class(new$batchDate) <- class(sEnd)
        attr(new$batchDate, "tzone") <- attr(sEnd, "tzone")   
