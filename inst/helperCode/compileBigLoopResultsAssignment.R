...
#       }
  ...
  for(i in 1:length(ls_sinks)){
    ...
    runs <- ifelse(runs == "fall","Late fall",runs)   # fix for Late fall, which is split into two entries.
    
    file <- left(right(ls_sinks[i],6),2)
    #by <- "All"
    
    runBL <- unlist(strsplit(ls_sinks[i],"--",fixed=TRUE))[c(TRUE,FALSE)]
    
    thisData <- data.frame(testi=runBL,file=file,run=runs,rows=rows)
...
all$rows <- format(round(as.numeric(levels(all$rows))[all$rows], 0),nsmall=0,big.mark=",")
