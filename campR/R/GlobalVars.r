...
#'   
#' @param Yes.code Integer defining the code for "Yes".  Usually 2. This code is
...
#'   
#' @param No.code Integer defining the code for "No".  Usually 1. This code is
#'   given in an Access look-up table named \code{table.names["yes.no.codes"]}.
#'   Unless it has changed, this table was named \code{luNoYes} in Access. To
#'   read this code from Access, an ODBC connection is established and
#'   disconnected during execution of this routine.
...
#'   date assignment, in military time. Default value is "04:00:00". This is
...
#'   
#' @param max.ok.gap Maximum gap, in hours, that is "okay". Default value is 2.
#'   Gaps in trapping smaller than this are ignored.  No catch is imputed for
#'   them. Gaps in trapping bigger than this get assigned an imputed catch from
#'   the GAM model.  Because the GAM model for imputation predicts an hourly
#'   rate, this max gap cannot be < 1 hour.
#'   
...
#'   Default value is 15. For example, \code{knotMesh == 15} means each
#'   additional knot in a smoothing spline requires an additional 15 data
#'   points.  A linear fit requires at least 15 data points, a quadratic
#'   requires at least 30 data points, a spline with 3 knots required 45, 4
#'   knots requires 60, etc. This restriction is put in place to help assure
#'   smoothing splines have adequate support and are stable.
#'   
#' @param halfConeMulti The multiplication factor to use for expanding fish
#'   caught during halfCone operations. Default value is 2. For example, if 15
#'   fish were caught during half cone operation, the analysis considers this 
#'   \code{halfConeMulti*15} fish.
#'   
#' @param sample.size.forkLength Number of fish with measured forklength
#'   required to assign life stage. Default value is 100.
#'   
#' @param sample.size.forkLengthAndWeight Number of fish with measured
#'   forklength \emph{and} weight required to use weight in the assignment of
#'   life stage.  Default value is 100.
#'   
#' @param weight.prop.forkLength The minimum proportion of fish with weight to
#'   fish with forklength in order to use weight in assignment of life stage.
#'   Default value is 0.5.
#'   
#' @param forkLength.mean.diff When the number of life stage groups (2 or 3) is
#'   not specified the clustering algorithm starts with 3 groups and reduces
#'   groups if the mean difference in forklength between groups is less than
#'   this number. Units are same as forklengths (mm). Default value is 10 mm.
#'   
#' @param time.zone Time zone to assign to all times.  Default value is
#'   "America/Los_Angeles", or Pacific time. It would likely never happen, but
...
#'   a random draw.  Currently set equal to 884.
#'   
...
#' these rarely if ever change, we purposefully left this variable out of the
#' arguments to \code{GlobalVars}. To change table names, you must edit the code
