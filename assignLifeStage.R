assignLifeStage <- function( site, taxon, run, min.date, max.date, output.file ){
#
...
## for debugging
## outfile <- "C:/Users/jstudyvin/GoogleDrive/project/PSMFS/fishLength/CAMP_RST20151014-DougEagleman/Outputs/"
## load(paste0(outfile,'input.Rdata'))
## source('getDataForLifeStageAssign.R')
## ls()
fileStruct <- unlist(strsplit(output.file,'/'))
filePath <- paste(fileStruct[-length(fileStruct)],collapse='/')
save(site,run,min.date,max.date,db.file,output.file,file=paste0(filePath,'/input','.Rdata'))
...
## get the data
data <- getDataForLifeStageAssign(taxon=taxon,site=site,min.date=min.date,max.date=max.date)
write.csv(data,paste0(output.file,'.csv'))
